...
We introduce the main notation used throughout this report:

\subsubsection{Spaces and Norms}
\begin{itemize}
    \item $H$: A separable Hilbert space (possibly infinite-dimensional)
    \item $\langle \cdot, \cdot \rangle$: The inner product on $H$
    \item $\zp{\cdot}$: The norm induced by the inner product on $H$
    \item $\E_{\mathcal{B}}$ or $\E^B$: The Bochner expectation for random variables in $H$
    \item $(\Omega, \mathcal{A}, \p)$: A complete probability space
\end{itemize}

\subsubsection{Random Variables and Distributions}
\begin{itemize}
    \item $Y: \Omega \to \R$: The real-valued response variable (heavy-tailed)
    \item $X: \Omega \to H$: The functional covariate in $H$
    \item $F$: The cumulative distribution function (cdf) of $Y$
    \item $\bar{F} = 1 - F$: The survival function
    \item $f = F'$: The density of $Y$ (when it exists)
    \item $F^-$: The generalized inverse (quantile function) of $F$
    \item $U(t) := F^-(1 - 1/t)$: The tail quantile function
    \item $Y_{1,n} \le Y_{2,n} \le \cdots \le Y_{n,n}$: The order statistics of the sample $(Y_i)_{1 \le i \le n}$
    \item $Y_{n-k+1,n}$: The $(n-k+1)$-th order statistic (the $k$-th largest value)
\end{itemize}

\subsubsection{Tail Moments and Regular Variation}
\begin{itemize}
    \item $m_W(y) := \E(W \mathbf{1}_{\{Y \ge y\}})$: The tail-moment of a random object $W$ (when it exists)
    \item $\hat{m}_W(y) := \frac{1}{n}\sum_{i=1}^n W_i \mathbf{1}_{\{Y_i \ge y\}}$: The empirical tail-moment
    \item $\RV_\tau(+\infty)$: The class of regularly-varying functions at infinity with index $\tau$
    \item $2\RV_{\gamma,\rho}(+\infty)$: The class of second-order regularly-varying functions
    \item $H_\rho(y) := y^{\gamma}\int_1^y u^{\rho-1}\mathrm{d}u$: The auxiliary function for second-order regular variation
    \item $A(t)$: An auxiliary function in the second-order condition, ultimately of constant sign with $A(t) \to 0$ as $t \to +\infty$
\end{itemize}

\subsubsection{Model and Estimators}
\begin{itemize}
    \item $\varphi$: A test function such that $\varphi \in \RV_\tau(+\infty)$ for some $\tau \in \R$
    \item $w_\varphi(y)$: The theoretical FEPLS direction (Functional Extreme-PLS)
    \item $\hat{\beta}_\varphi(y)$: The empirical FEPLS estimator of the direction
    \item $g$: A deterministic link function such that $g \in \RV_\kappa(+\infty)$ for some $\kappa > 0$
    \item $\beta \in H$: The true index direction with $\zp{\beta} = 1$
    \item $\varepsilon: \Omega \to H$: A noise random variable
    \item $k = k_n$: An intermediate sequence (integer such that $k \to +\infty$ and $k/n \to 0$)
    \item $y_{n,k}$: A deterministic sequence such that $y_{n,k} \sim U(n/k)$ as $n \to +\infty$
    \item $\delta_{n,k}$: The convergence rate, defined as $\delta_{n,k} := (g(y_{n,k})(k/n)^{1/q})^{-1}$
    \item $q > 2$: The integrability order of the noise (independent of $n$)
\end{itemize}

\subsubsection{Model Parameters}
\begin{itemize}
    \item $\gamma \in (0,1)$: The tail index of $Y$ (first-order parameter)
    \item $\rho \le 0$: The second-order parameter controlling the rate of convergence
    \item $\kappa > 0$: The index of regular variation of the link function $g$
    \item $\tau \in \R$: The index of regular variation of the test function $\varphi$
\end{itemize}
...
\subsection{The Role of Regular Variation: Why Power Laws Matter}

Regular variation is the mathematical language for describing ``power law'' behavior. A function $f$ is regularly varying with index $\alpha$ if, for large $t$, it behaves like $f(t) \approx t^{\alpha} \times \text{slowly varying function}$.

\textbf{Why this matters:}
\begin{itemize}
    \item Heavy-tailed distributions (like those in finance) have power-law tails: $P(Y > y) \approx y^{-1/\gamma}$.
    \item The link function $g$ also has power-law behavior: $g(y) \approx y^{\kappa}$.
    \item This allows us to use the powerful machinery of extreme value theory, which is built on regular variation.
    \item The composition of regularly-varying functions is also regularly varying, which makes the mathematics tractable.
\end{itemize}
...
\subsection{Explicit Solution and Generalization}

The solution to the optimization problem \eqref{eq:fepls} can be explicitly characterized. More generally, we consider a family of estimators parameterized by a test function $\varphi$:

\begin{theorem}[Explicit Solution of the FEPLS Optimization Problem]
\label{thm:solution}
...
\begin{align}
\label{eq:solution}
w(y) &= \frac{v(y)}{\zp{v(y)}} \quad \text{where} \quad v(y) = \bar{F}(y) m_{XY}(y) - m_X(y) m_Y(y),
\end{align}
where $\bar{F}(y) = 1 - F(y)$ is the survival function and $m_W(y) = \E(W \mathbf{1}_{\{Y \ge y\}})$ denotes the tail-moment of a random object $W$.

More generally, we consider the FEPLS direction:
\begin{align}
\label{eq:solution2}
w_\varphi(y) &= \frac{v_\varphi(y)}{\zp{v_\varphi(y)}} \quad \text{with} \quad v_\varphi(y) := m_{X\varphi(Y)}(y),
\end{align}
where $\varphi$ is a test function that is regularly-varying at infinity with index $\tau \in \R$, i.e., $\varphi \in \RV_\tau(+\infty)$.
\end{theorem}
...
\subsection{The Empirical FEPLS Estimator}

Given a sample $(X_i, Y_i)_{1 \le i \le n}$ of independent copies of $(X, Y)$, we define the empirical tail-moment:
$$
\hat{m}_W(y) := \frac{1}{n}\sum_{i=1}^n W_i \mathbf{1}_{\{Y_i \ge y\}}.
$$

The FEPLS estimator is then:
\begin{align}
\label{eq:estimator}
\hat{\beta}_\varphi(y) := \frac{\hat{v}_\varphi(y)}{\zp{\hat{v}_\varphi(y)}} \quad \text{with} \quad \hat{v}_\varphi(y) = \hat{m}_{X\varphi(Y)}(y) = \frac{1}{n} \sum_{i=1}^n X_i \varphi(Y_i) \mathbf{1}_{\{Y_i \ge y\}}.
\end{align}

\subsection{Main Consistency Theorem}

\begin{theorem}[Consistency of FEPLS Estimator]
\label{thm:consistency}
Assume the following conditions:
\begin{enumerate}
    \item The test function $\varphi \in \RV_\tau(+\infty)$ for some $\tau \in \R$
    \item The link function $g \in \RV_\kappa(+\infty)$ for some $\kappa > 0$
    \item The tail quantile function $U \in 2\RV_{\gamma,\rho}(+\infty)$ with $\gamma \in (0,1)$ and $\rho \le 0$
    \item The noise condition: there exists $q > 2$ such that
    \begin{align}
    \label{hyp:noise_cond}
    \limsup_{n \to +\infty} \sup_{y \ge 0} \E\left(\zp{\varepsilon_1}^{q} \mid Y_{n-k+1,n} = y\right) < +\infty
    \end{align}
    \item The threshold condition: $\sqrt{k}A(n/k) = O(1)$
    \item The bounds:
    \begin{align}
    \label{hyp:2cgamma}
    ...
    q\kappa\gamma > 1
    \end{align}
    \item Regularity: $\varphi$ and $g$ are continuously differentiable in a neighbourhood of infinity such that $t(\varphi g)'(t)/(\varphi g)(t) \to \tau + \kappa \neq 0$ as $t \to +\infty$
\end{enumerate}

Let $k := k_n \to +\infty$ be an intermediate sequence (i.e., $k/n \to 0$ as $n \to +\infty$), and let $y_{n,k} \sim U(n/k)$ as $n \to +\infty$. Define the convergence rate:
$$
\delta_{n,k} := \left(g(y_{n,k})\left(\frac{k}{n}\right)^{1/q}\right)^{-1}.
$$

Then, the FEPLS estimator satisfies:
$$
\zp{ \hat{\beta}_\varphi(Y_{n-k+1,n}) - \beta } = O_{\p}(\delta_{n,k}) \xrightarrow[n \to +\infty]{} 0.
$$
\end{theorem}

\begin{boundbox}{Interpretation of the Bounds}
The consistency theorem requires two key bounds that ensure the estimator converges:

\textbf{Bound 1: } $0 < 2(\kappa + \tau)\gamma < 1$

This condition ensures the existence of the second moment of $\varphi \cdot g(Y) \mathbf{1}_{\{Y \ge y\}}$ for all $y \ge 0$. 

\begin{itemize}
    \item The product $(\kappa + \tau)\gamma$ represents the effective tail index of the weighted response $\varphi(Y) \cdot g(Y)$
    \item The factor of 2 comes from requiring the second moment to exist
    \item The upper bound $< 1$ ensures that the variance remains finite in the tail
    \item This implies that $\tau < \frac{1}{2\gamma} - \kappa$, which can be either positive or negative depending on $\gamma$ and $\kappa$
\end{itemize}

\textbf{Bound 2: } $q\kappa\gamma > 1$

This condition ensures that the convergence rate $\delta_{n,k} \to 0$ as $n \to +\infty$, regardless of the slowly-varying parts of $g$ and $\bar{F}$.

\begin{itemize}
    \item The product $\kappa\gamma$ is the tail index of $g(Y)$
    \item The factor $q$ (where $q > 2$ from the noise condition) ensures sufficient integrability
    \item This condition implies that $g(Y)$ has a heavier tail than $\|\varepsilon\|$, which is necessary for the signal to dominate the noise in the extreme regime
    \item The convergence rate $\delta_{n,k}$ is of order $n^{(1/q - \gamma\kappa)/(1-2\rho)}$, so this bound ensures the exponent is negative
\end{itemize}

Together, these bounds ensure that the FEPLS estimator is both well-defined and consistent.

\textbf{Summary:}
\begin{itemize}
    \item $q\kappa\gamma > 1$: Extract signal from noise (signal tail heavier than noise tail)
    \item $2(\kappa + \tau)\gamma < 1$: Ensure $\hat{\beta}$ is consistent and convergent (variance remains finite)
\end{itemize}
\end{boundbox}

\subsection{Technical Lemmas}

The consistency proof relies on several technical lemmas that we state here for completeness. These lemmas handle the technical aspects of working with random thresholds and controlling the noise terms.

\begin{lemma}[Bochner Expectation and Inner Product Commutation]
\label{lem:bochner_random}
...
\begin{remark}
This lemma is crucial for manipulating conditional expectations involving inner products in Hilbert spaces. It essentially says that when two random variables are conditionally independent, the expectation of their inner product equals the inner product of their expectations. This is the infinite-dimensional analog of the fact that $\E(XY) = \E(X)\E(Y)$ for independent random variables.
\end{remark}

\begin{lemma}[Joint Density of Order Statistics]
\label{lem:density}
Let $\{Y_1,\dots,Y_n\}$ be i.i.d. random variables with common cdf $F$. Let $k$ be an integer such that $2 \le k < n$ and let any $1 \le i \le n$. The joint density $(t,y) \mapsto f_{n,k}(t,y)$ of $(Y_i, Y_{n-k+1,n})$ is given for all $y \le t$ by:
\[
f_{n,k}(t,y) = f(t)f(y)\frac{(n-1)!}{(n-k)!(k-2)!} F^{n-k}(y)\bar{F}^{k-2}(y).
\]
\end{lemma}

\begin{lemma}[Conditional Expectation with Random Threshold]
\label{lem:bousebata_random}
Let $h \in \RV_{\rho}(+\infty)$ with $\rho \in \R$ and i.i.d. random variables $\{Y_1,\dots,Y_n\}$ with common density $f \in \RV_{-1/\gamma -1}(+\infty)$, $\gamma > 0$. Assume that $\rho\gamma < 1$. Let $2 \le k \le n$ be some integer. Then, for any $y \ge 0$ and any $1 \le i \le n$:
\[
...
\]
Moreover, when $y \to +\infty$:
\[
\E\left(h(Y_i)\mathbf{1}_{\{Y_i \ge Y_{n-k+1,n}\}} \mid Y_{n-k+1,n} = y\right) \sim \frac{k-1}{n} \cdot \frac{h(y)}{1 - \rho\gamma}.
\]
\end{lemma}

\begin{lemma}[Bound on Empirical Tail-Moments of Noise]
\label{lem:norm_noise_v2_random}
Assume that $\bar{F} \in \RV_{-1/\gamma}(+\infty)$, the test function condition, link function condition, noise condition, and the bounds hold. Let $k := k_n \to +\infty$ be an integer deterministic sequence such that $k/n \to 0$ and $y_{n,k} \sim U(n/k)$ as $n \to +\infty$. Let $\delta_{n,k} := (g(y_{n,k})(k/n)^{1/q})^{-1}$. Then:
\[
\frac{\zp{\hat{m}_{\varphi(Y)\varepsilon}(Y_{n-k+1,n})}}{m_{\varphi \cdot g(Y)}(y_{n,k})} = O_{\p}(\delta_{n,k}) \xrightarrow[n \to +\infty]{} 0.
\]
\end{lemma}

\begin{lemma}[Bound on Inner Product with Noise]
\label{lem:inner_prod_noise_L2_random}
Let $\beta \in H$ with $\zp{\beta} = 1$ and suppose $\bar{F} \in \RV_{-1/\gamma}(+\infty)$. Assume the test function, link function, noise condition, and bounds hold. Let $k := k_n \to +\infty$ be an integer deterministic sequence such that $k/n \to 0$ and $y_{n,k} \sim U(n/k)$ as $n \to +\infty$. Let $\delta_{n,k} := (g(y_{n,k})(k/n)^{1/q})^{-1}$. Then:
\[
\frac{\hat{m}_{\langle \beta, \varphi(Y)\varepsilon \rangle}(Y_{n-k+1,n})}{m_{\varphi \cdot g(Y)}(y_{n,k})} = O_{\p}(\delta_{n,k}) \xrightarrow[n \to +\infty]{} 0.
\]
\end{lemma}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/4ig_akko_tau_comparison.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/4ig_akko_tau_0.0.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/akko_4ig_tau_comparison.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/akko_4ig_tau_-1.0.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/4ig_akko_tau_-0.5.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/AAPL_1ms_d20_k10_tau_comparison.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{figures/AAPL_1sec_d50_k20_tau_comparison.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/AAPL_100ms_d20_k10_tau_0.0.png}
