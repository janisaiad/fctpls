...
Following the classical extreme-value theory framework (see \cite[Equation~3.2.10]{de2007}), the optimal choice of $k$ balances bias (governed by 2RV, i.e., $A(n/k)$) and variance (proportional to $k^{-1/2}$):
\begin{quote}
\[
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.3\textwidth]{figures/4ig_akko_hill_qq_tail.png}
...
This section presents detailed FEPLS analysis results for 4IG paired with multiple stocks from the Hungarian market. 
We chose this stocks because many estimation were consistent and some others were not, so that our report is representative of what you can get with real data without any well designed experiments to make the framework to work.
...
\subsubsection{Hill, Q-Q, and Tail Plots}
...
\begin{subfigure}[b]{0.32\textwidth}
\centering
\includegraphics[width=\textwidth]{figures/4ig_akko_tau_-2.0_0.0.png}
\caption{$(\tau_1, \tau_2) = (-2.0, 0.0)$}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.32\textwidth}
\centering
