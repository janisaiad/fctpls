...
def stooq_txt_to_df_daily(file_path: Union[str, Path], tz: Optional[str] = None) -> pd.DataFrame:
    '''
    we parse a stooq txt file and return only daily bars with validated timestamps  # we describe the purpose
    ...
    valid = (df.index.hour == 0) & (df.index.minute == 0) & (df.index.second == 0)  # we validate timestamps align to midnight
...
def curate_stooq_many_daily(
    ...
    '''
    we parse multiple files keeping only daily bars  # we describe the purpose
    '''
    out: Dict[str, pd.DataFrame] = {}  # we prepare output mapping
    for path in paths:  # we iterate files
        try:
...
def curate_stooq_dir_daily(
    ...
    '''
    we list files and parse only daily bars  # we describe the purpose
    '''
    files = list_txt_files(dir_path, pattern=pattern, recursive=recursive)  # we list files
